...
conn.cursor().execute("""
    CREATE OR REPLACE TABLE TD_ALL_EVENTS (
        ...
        event_type_logo TEXT,
        tags TEXT,
        allows_cohosting BOOLEAN,
        chapter_chapter_location TEXT,
        chapter_city TEXT,
        chapter_country TEXT,
        chapter_country_name TEXT,
        chapter_description TEXT,
        chapter_id TEXT,
        chapter_hide_country_info BOOLEAN,
        chapter_logo TEXT,
        chapter_state TEXT,
        chapter_timezone TEXT,
        chapter_title TEXT,
        chapter_relative_url TEXT,
        chapter_url TEXT
    );
""")

conn.cursor().execute("PUT file://events.*csv @TD_STAGE;")

conn.cursor().execute("COPY INTO TD_ALL_EVENTS FROM @TD_STAGE;")

conn.cursor().execute("ALTER SESSION SET date_output_format = 'MMMM DD, YYYY';")
    ...
    CREATE OR REPLACE TABLE td_accessible_events AS (
        WITH main_data AS (
            SELECT
                ...
                TO_TIMESTAMP(LEFT(split_part(start_date, 'T', 2), 8), 'HH24:MI:SS') AS local_time,
                CASE
                    WHEN POSITION('Z' IN start_date) > 0 THEN
                        CASE
                            WHEN MONTH(to_date(split_part(start_date, 'T', 1))) BETWEEN 4 AND 10 THEN
                                '+01:00'
                            ELSE
                                '+00:00'
                        END
            ...
            *,
            CASE
                WHEN event_type_title IN ('Virtual Event', 'Virtual', 'Hybrid Event', 'Hybrid')
                    AND gmt_time NOT BETWEEN TO_TIMESTAMP('02:00', 'HH24:MI') AND TO_TIMESTAMP('08:00', 'HH24:MI') THEN
                    TRUE
                ELSE
                    CASE
                        WHEN chapter_country_name IN (SELECT country FROM cities)
                            AND time_difference IN ('+00:00', '+01:00', '+02:00', '-04:00') THEN
                            TRUE
                        ELSE
                            FALSE
                    END
        ...
        FROM (
            SELECT
                *,
                LEFT(
                    CASE
                        WHEN LEFT(time_difference, 1) = '+' THEN
                            DATEADD(HOUR, -CAST(SUBSTR(time_difference, 3, 1) AS INT), local_time)
                        ELSE
                            DATEADD(HOUR, CAST(SUBSTR(time_difference, 3, 1) AS INT), local_time)
                    END, 5) AS gmt_time
            FROM main_data
